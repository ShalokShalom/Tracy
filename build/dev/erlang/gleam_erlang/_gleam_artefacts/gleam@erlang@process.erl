...
-opaque subject(RK) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(RK)}.

-type name(RL) :: any() | {gleam_phantom, RL}.

-type do_not_leak() :: any().

-type selector(RM) :: any() | {gleam_phantom, RM}.
...
-spec 'receive'(subject(SK), integer()) -> {ok, SK} | {error, nil}.
...
-spec receive_forever(subject(SS)) -> SS.
...
-spec selector_receive(selector(SW), integer()) -> {ok, SW} | {error, nil}.
...
-spec selector_receive_forever(selector(TA)) -> TA.
...
-spec map_selector(selector(TC), fun((TC) -> TE)) -> selector(TE).
map_selector(A, B) ->
...
-spec merge_selector(selector(TG), selector(TG)) -> selector(TG).
merge_selector(A, B) ->
...
-spec select_map(selector(TR), subject(TT), fun((TT) -> TR)) -> selector(TR).
...
-spec select(selector(TN), subject(TN)) -> selector(TN).
...
-spec select_other(selector(UF), fun((gleam@dynamic:dynamic_()) -> UF)) -> selector(UF).
...
-spec deselect(selector(TW), subject(any())) -> selector(TW).
...
-spec select_specific_monitor(selector(UR), monitor(), fun((down()) -> UR)) -> selector(UR).
...
-spec select_monitors(selector(UU), fun((down()) -> UU)) -> selector(UU).
...
-spec select_trapped_exits(selector(TK), fun((exit_message()) -> TK)) -> selector(TK).
...
-spec deselect_specific_monitor(selector(UX), monitor()) -> selector(UX).
...
-spec send_after(subject(VT), integer(), VT) -> timer().
...
-spec perform_call(
    subject(VA),
    fun((subject(VC)) -> VA),
    fun((selector(VC)) -> {ok, VC} | {error, nil})
) -> VC.
...
-spec call(subject(VH), integer(), fun((subject(VJ)) -> VH)) -> VJ.
...
-spec call_forever(subject(VL), fun((subject(VN)) -> VL)) -> VN.
