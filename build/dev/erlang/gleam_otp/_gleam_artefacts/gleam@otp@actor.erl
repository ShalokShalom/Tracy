...
-type message(ABV) :: {message, ABV} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(ABW, ABX) :: {continue,
        ABW,
        gleam@option:option(gleam@erlang@process:selector(ABX))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(ABY, ABZ) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        ABY,
        gleam@erlang@process:selector(message(ABZ)),
        gleam@otp@system:debug_state(),
        fun((ABY, ABZ) -> next(ABY, ABZ))}.

-type started(ACA) :: {started, gleam@erlang@process:pid_(), ACA}.

-opaque initialised(ACB, ACC, ACD) :: {initialised,
        ACB,
        gleam@option:option(gleam@erlang@process:selector(ACC)),
        ACD}.

-opaque builder(ACE, ACF, ACG) :: {builder,
        fun((gleam@erlang@process:subject(ACF)) -> {ok,
                initialised(ACE, ACF, ACG)} |
            {error, binary()}),
        integer(),
        fun((ACE, ACF) -> next(ACE, ACF)),
        gleam@option:option(gleam@erlang@process:name(ACF))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(ACH) :: {ack, {ok, ACH} | {error, binary()}} |
...
-spec continue(ACM) -> next(ACM, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(ACY, ACZ), gleam@erlang@process:selector(ACZ)) -> next(ACY, ACZ).
...
-spec initialised(ADF) -> initialised(ADF, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(ADK, any(), ADM),
    gleam@erlang@process:selector(ADQ)
) -> initialised(ADK, ADQ, ADM).
...
-spec returning(initialised(ADV, ADW, any()), AEB) -> initialised(ADV, ADW, AEB).
...
-spec new(AEF) -> builder(AEF, AEG, gleam@erlang@process:subject(AEG)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(AEL)) -> {ok, initialised(AEN, AEL, AEO)} |
        {error, binary()})
) -> builder(AEN, AEL, AEO).
...
-spec on_message(builder(AEX, AEY, AEZ), fun((AEX, AEY) -> next(AEX, AEY))) -> builder(AEX, AEY, AEZ).
...
-spec named(builder(AFI, AFJ, AFK), gleam@erlang@process:name(AFJ)) -> builder(AFI, AFJ, AFK).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(AFX))) -> gleam@erlang@process:selector(message(AFX)).
...
-spec receive_message(self(any(), AFT)) -> message(AFT).
...
-spec initialise_actor(
    builder(any(), any(), AGQ),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, AGQ} | {error, binary()})
...
-spec start(builder(any(), any(), AHD)) -> {ok, started(AHD)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(AHK), AHK) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(AHM),
    integer(),
    fun((gleam@erlang@process:subject(AHO)) -> AHM)
) -> AHO.
