...
-type child_specification(AVA) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(AVA)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(AVB)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(AVB).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(AVG)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(AVG).
supervisor(Start) ->
...
-spec significant(child_specification(AVL), boolean()) -> child_specification(AVL).
...
-spec timeout(child_specification(AVO), integer()) -> child_specification(AVO).
timeout(Child, Ms) ->
...
-spec restart(child_specification(AVR), restart()) -> child_specification(AVR).
...
-spec map_data(child_specification(AVU), fun((AVU) -> AVW)) -> child_specification(AVW).
