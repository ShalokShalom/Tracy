...
-opaque supervisor(AXO, AXP) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(AXO, AXP))}.

-type message(AXQ, AXR) :: any() | {gleam_phantom, AXQ, AXR}.

-opaque builder(AXS, AXT) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((AXS) -> {ok, gleam@otp@actor:started(AXT)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(AXS, AXT)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(AXU, AXV) :: {local,
        gleam@erlang@process:name(message(AXU, AXV))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(AXW) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, AXW}.
...
-type erlang_child_spec_property(AXX, AXY) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((AXX) -> {ok, gleam@otp@actor:started(AXY)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(AXZ, AYA))) -> supervisor(AXZ, AYA).
get_by_name(Name) ->
...
-spec named(builder(AYQ, AYR), gleam@erlang@process:name(message(AYQ, AYR))) -> builder(AYQ, AYR).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(AYZ, AZA), integer(), integer()) -> builder(AYZ, AZA).
...
-spec timeout(builder(AZF, AZG), integer()) -> builder(AZF, AZG).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(AZL, AZM), gleam@otp@supervision:restart()) -> builder(AZL, AZM).
...
-spec start(builder(AZR, AZS)) -> {ok,
        gleam@otp@actor:started(supervisor(AZR, AZS))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(BAQ, BAR)) -> gleam@otp@supervision:child_specification(supervisor(BAQ, BAR)).
supervised(Builder) ->
...
-spec start_child(supervisor(BAX, BAY), BAX) -> {ok,
        gleam@otp@actor:started(BAY)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((BBV) -> {ok, gleam@otp@actor:started(BBW)} |
        {error, gleam@otp@actor:start_error()}),
    BBV
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), BBW, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((AYG) -> {ok, gleam@otp@actor:started(AYH)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(AYG, AYH).
...
-spec supervisor_child(
    fun((AYL) -> {ok, gleam@otp@actor:started(AYM)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(AYL, AYM).
